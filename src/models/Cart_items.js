...
Cart_items_Schema.statics.isThisCart = async function (product_id, user_id, color) {
    if (!product_id) throw new Error('Invalid product_id')
    try {
        const product = await this.findOne({ product_id: product_id, user_id: user_id, color: color })

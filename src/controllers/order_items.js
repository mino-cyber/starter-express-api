...
module.exports.Create_order_item = async (req, res) => {
    ...
    for (var i = 0; i < body.products.length; i++) {
        ...
        await Product.findById(body.products[i].product_id).then(async (product) => {
            ...
            const index = product.colors.indexOf(body.products[i].color)
            await Product.findByIdAndUpdate(body.products[i].product_id, { $set: { [`quantity.${index}`]: product.quantity[index] - body.products[i].quantity } }).then(async (product1) => {

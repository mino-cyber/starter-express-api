...
module.exports.Create_cart_item = async (req, res) => {
    ...
    const isNewCart = await Cart_items.isThisCart(body.product_id, id, body.color)
    if (!isNewCart) {
        return res.json({
...
const add_cart_item = async ({ product_id, color, quantity }, id) => {
    ...
    await Cart_items.findById(_id).then(async (e) => {
        if (!e) {
            ...
            await Product.findById(e.product_id).then(async (product) => {
                const index = product.colors.indexOf(body.color)
                await Cart_items.updateOne({ _id: _id, quantity: { $lt: product.quantity[index] } }, { $inc: { quantity: 1 } }, { new: true }).then(e => {
                    return res.status(200).json(e)
                }).catch(err => {
                    console.log(err.message)
                    return res.status(401).json({ error: err.message })
